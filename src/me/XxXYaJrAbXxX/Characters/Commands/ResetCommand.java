		...
		try {
			if (args[1].equalsIgnoreCase(null)) {
				...
				if ((Bukkit.getOnlinePlayers().contains(Bukkit.getPlayer(args[1])) == true)
					...
					SetData.generatePlayerData(args[1]);
					sender.sendMessage(ChatColor.GREEN + "Their character templates were reset and saved!");
					Bukkit.getPlayer(args[1])
							.sendMessage(ChatColor.GREEN + "Your character templates were reset and saved!");
				} else {
			...
			sender.sendMessage(ChatColor.GREEN + "Your character templates were reset and saved!");
		}
	}
}